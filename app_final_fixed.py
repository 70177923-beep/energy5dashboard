...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    latest_df["solar_share_elec"] = latest_df["solar_share_elec"].fillna(0)
    latest_df["wind_share_elec"] = latest_df["wind_share_elec"].fillna(0)

    latest_df = latest_df.dropna(
        subset=["country", "renewables_share_energy"]
    )
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with col_right:
        ...
        fos_top = df[df["year"] == latest_yr].copy()
        fos_top = fos_top.dropna(subset=["country","fossil_share_energy"])
        fos_top = fos_top.nlargest(10, "fossil_share_energy")
        fig8, ax8 = dark_fig((6, 3.5))
        if not fos_top.empty:
            ax8.barh(fos_top["country"], fos_top["fossil_share_energy"], color="#ff6b35", alpha=0.85, edgecolor="#0b1a2b")
            ax8.set_xlabel("Fossil Share (%)")
            st.pyplot(fig8)
        else:
            st.warning("No fossil-fuel data available for selected year.")
